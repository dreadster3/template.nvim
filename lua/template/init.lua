...
function temp.get_temp_list()
    ...
    local result = vim.fs.find(function(name) return name:match('.*') end, {
        type = 'file',
        path = temp.temp_dir,
        limit = math.huge
    })

    local link = vim.fs.find(function(name) return name:match('.*') end, {
        type = 'link',
        path = temp.temp_dir,
        limit = math.huge
    })
    ...
    for _, name in ipairs(result) do
        ...
        if ft then
            if not res[ft] then res[ft] = {} end
            res[ft][#res[ft] + 1] = name
        else
            vim.notify(
                '[Template.nvim] Could not find the filetype of template file ' ..
                    name, vim.log.levels.INFO)
        end
    end
...
local function expand_expr()
    local expr = {
        '{{_date_}}', '{{_cursor_}}', '{{_file_name_}}', '{{_author_}}',
        '{{_email_}}', '{{_dir_}}', '{{_upper_file_}}', '{{_lua:(.-)_}}',
        '{{_tomorrow_}}', '{{_(.-)_}}'
    }
    ...
    local function expand_recursively(line, expr, replacement)
        while line:match(expr) do line = line:gsub(expr, replacement, 1) end
        return line
    end
    ...
    local expr_map = {
        ...
        [expr[6]] = function(line)
            ...
            return expand_recursively(line, expr[6], dir)
        end,
        -- [expr[6]] = function(line)
        --     if not line:match(expr[6]) then return line end
        ...
        [expr[8]] = function(line)
            while line:match(expr[8]) do
                line = line:gsub(expr[8],
                                 load('return ' .. line:match(expr[8]))(), 1)
                print(line)
            end
            return line
        end,
        ...
        [expr[10]] = function(line)
            local var = line:match(expr[10])
            local value = ""
            if not table.contains(variables, var) then
                variables[var] = vim.fn.input("Variable " .. var .. " value: ")
            end

            return expand_recursively(line, expr[10], variables[var])
        end
    ...
    return function(line)
        ...
        if line:find(expr[2]) then cursor = true end

        for i, item in ipairs(expr) do line = expr_map[item](line) end
...
local function parse_args(args)
    ...
    for _, v in pairs(args) do
        if v:find('%.%w+') then data.file = v end
        data.tp_name = v
    end
...
local function get_tpl(buf, name)
    ...
    if not list[vim.bo[buf].filetype] then return end

    for _, v in ipairs(list[vim.bo[buf].filetype]) do
        if v:find(name) then return v end
    end
end
...
function temp:generate_template(args)
    ...
    if data.file then create_and_load(data.file) end
    ...
    async_read(tpl, ---@diagnostic disable-next-line: redefined-local
    vim.schedule_wrap(function(data)
        ...
        for i, v in ipairs(tbl) do
            ...
            lines[#lines + 1] = line
            if cursor then cursor_pos = {i - skip_lines, 2} end
            ::continue::
        end
...
function temp.in_template(buf)
    ...
    if vim.tbl_contains(list[vim.bo[buf].filetype], bufname) then return true end
